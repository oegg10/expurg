...
if (!isset($_SESSION['idusuario'])) {
    ...
    if (!empty($_POST)) {
        ...
        if (!empty($errores)) {
            ...
            $cancelar = "UPDATE recepciones SET mtvocancelo = '$mtvocancelo', condicion = 3, idusuario = '$idusuario', fechamod = NOW() WHERE idrecepcion = '$id'";
            $cancelado = $con->query($cancelar);

            if ($cancelado > 0) {

                header('location:../extend/alerta.php?msj=La consulta fue catalogada como NO CONSULTADO&c=cons1&p=in&t=success');
                $con->close();
                exit();
            } else {

                header('location:../extend/alerta.php?msj=La recepcion no se pudo realizar&c=cons1&p=in&t=error');
            }
...
?>
                        ...
                        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                            <form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" id="formConsulta" autocomplete="off">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-lg-12 col-md-12 col-sm-12">
                                        <label>Motivo de la cancelación de la consulta (*): <strong style="color: red;">Solo acepta letras y espacios</strong></label>
                                        <input type="text" class="form-control" name="mtvocancelo" id="mtvocancelo" style="background: black; color: white" onkeypress="return check(event)" placeholder="Motivo de la cancelación de la consulta" onblur="may(this.value, this.id)" required>
                                    </div>
    ...
    <script>
        ...
        mtvocancelo.addEventListener("keypress", validarCampo);
